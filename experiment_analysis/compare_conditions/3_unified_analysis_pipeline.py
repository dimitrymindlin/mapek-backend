"""Unified analysis pipeline for comparing two sanitised conditions."""
...
# Ensure local modules and the repository root are importable when executed as a script
...
from dialogue_analysis_base import QuestionnaireAnalyzer, UnifiedDataLoader  # type: ignore  # noqa: E402
from experiment_analysis.plot_overviews import extract_feedback_data  # type: ignore  # noqa: E402
import likert_scale_analysis_refactored  # noqa: E402
from analyse_dialogues_refactored import get_suggestions, is_autosuggested_question  # type: ignore  # noqa: E402
...
class AnalysisConfig:
    ...
    def _resolve_config_path(self, config_path: str) -> Path:
        """Convert the provided config path to an absolute path."""
        path_obj = Path(config_path)

        if path_obj.is_absolute() and path_obj.exists():
            return path_obj

        for base_dir in (CURRENT_DIR, ROOT_DIR, Path.cwd()):
...
class UnifiedAnalysisPipeline:
    ...
    def setup_logging(self):
        ...
        log_file = self._resolve_path(self.config.config['logging']['file'])
    ...
    def _resolve_path(self, path_value: Any) -> Path:
        """Resolve a path value relative to the project root when needed."""
        path_obj = Path(path_value) if not isinstance(path_value, Path) else path_value
        return path_obj if path_obj.is_absolute() else (self.project_root / path_obj).resolve()
    ...
    def _load_data_frames(self) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        def build_condition_dir(key: str) -> Path:
            base_dir = self._resolve_path(data_sources[key])
            return base_dir / filtered_subdir if filtered_subdir else base_dir

        if exp_settings['comparison_mode']:
            condition_dirs = {
                exp_settings['condition_a']: str(build_condition_dir('condition_a_dir')),
                exp_settings['condition_b']: str(build_condition_dir('condition_b_dir'))
            }
        else:
            # For single condition, use condition_a_dir
            condition_dirs = {
                exp_settings['single_condition']: str(build_condition_dir('condition_a_dir'))
            }
    ...
    def run_all_analyses(self) -> Dict[str, Any]:
        ...
        if exp_settings['comparison_mode']:
            directories_used = [
                str(self._resolve_path(data_sources['condition_a_dir'])),
                str(self._resolve_path(data_sources['condition_b_dir']))
            ]
        else:
            directories_used = [str(self._resolve_path(data_sources['condition_a_dir']))]
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        config_path = CURRENT_DIR / '1_analysis_config.yaml'

        if not config_path.exists():
            print(f"Creating configuration: {config_path}")
            AnalysisConfig(str(config_path))

        pipeline = UnifiedAnalysisPipeline(str(config_path))
    ...
    parser.add_argument('--config', default=str(CURRENT_DIR / '1_analysis_config.yaml'), help='Config file path')
